...
#define BUTTON_PIN PD6 // Digital 6 in UNO
	...
	while (1)
	{
		...
		for(int8_t spi_data_index = 0; spi_data_index < sizeof(spi_send_data); spi_data_index++) //to read data from master
		{
			...
			if(spi_receive_data[spi_data_index] == 'O'){
				...
				if (!(PIND & (1<<BUTTON_PIN))){ // PIND checks the current bit situation and if button is pressed it is 0 but second part is 1 so to make the condition true we should get the negative of it
					set_as_output(&obstacle_led); // makes pin output
					
					uint8_t count=0; // counts the blinking amount
					while (count<3){
						set_gpio(&obstacle_led); // LED ON
						DELAY_sec(FAULT_BLINK_PERIOD_MS); // blinking time
						clear_gpio(&obstacle_led); // LED OFF
						count++; // increment to count the blinking
					}
					spi_uno_send((uint8_t*)obstacle_detection_command,strlen(obstacle_detection_command));
